...
columns = ["Open", "Close", "High", "Low", "Volume", "CPI.csv", "FEDFUNDS.csv", "SavingsRate.csv", "UNRATE.csv"]
for column in columns:
    ...
    result = np.append(result, extra_prediction)
...
for column in columns:
    ...
    df_stock = yf.Ticker("AAPL").history(start="2022-01-01", end="2024-01-22", interval="1d").reset_index()[
        ["Date", "Open", "Close", "High", "Low", "Volume"]]
    ...
    #nr1 extra prediction
    extra_test_pred = np.append(scaled_x_data[-1:], result)
    extra_test_pred = extra_test_pred.reshape((1, -1))
    extra_pred = best_model.predict(extra_test_pred)
    extra_prediction = extra_pred.reshape((-1, 1))
    result1 = np.append(result1, extra_prediction)
    extra_ennustus = scaler.inverse_transform(extra_prediction)
    predictions.append(extra_ennustus[0][0])
    print(extra_ennustus)

"""
#ennustab np.append(scaled_x_data[-1:], result)
